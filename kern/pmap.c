...
static int page_walk_free_list();
...
mem_init(void)
{
    ...
    //cprintf("before check_page_free_list(1)\n");
	check_page_free_list(1);
    //cprintf("after check_page_free_list(1)\n");
	check_page_alloc();
    //cprintf("after check_page_alloc()\n");
    //page_walk_free_list();
	check_page();
...
static int _page_calc_free(int page_seq, int kern_stop)
{
    ...
    else if(page_seq >= iophysmem && page_seq <= extphysmem) { // 3)
        free = 0;
    }
...
page_init(void)
{
	...
	// free pages!
	size_t i;
    int kern_stop = ROUNDUP(PADDR(boot_alloc(0)), PGSIZE) / PGSIZE;
    //cprintf("npages %d kern_stop 0x%x\n",npages, kern_stop);
	for (i = 0; i < npages; i++) {
		if(!_page_calc_free(i, kern_stop)) {
            continue;
        }
        pages[i].pp_ref = 0;
...
page_walk_free_list()
{
    ...
    cprintf("page_walk_free_list 0x%x\n",page_free_list);
    while (page) {
        cprintf("  page 0x%x ref %d\n",page, page->pp_ref);
        page = page->pp_link;
    }
...
page_alloc(int alloc_flags)
{
	...
	/* lj */
	struct Page *page = page_free_list;
    struct Page *prev = page;
    //cprintf("page_alloc list 0x%x\n", page);

    while (page) {
    //cprintf("\tpage_alloc = 0x%x next 0x%x ref %d\n", page, page->pp_link, page->pp_ref);
        if(0 == page->pp_ref) {
            break;
        }
        prev = page;
...
page_free(struct Page *pp)
{
	// Fill this function in
    /* lj */
    //cprintf("free pp 0x%x ref %d\n", pp, pp->pp_ref);
    if(pp->pp_ref) {
        return;
    }
	pp->pp_link = page_free_list;
	page_free_list = pp;
    //cprintf("after free pp 0x%x  list 0x%x ref %d\n", pp, page_free_list,  pp->pp_ref);
    //page_walk_free_list();
}
...
//	lj:Note that "into" above line, we should return pte, not a 
//	pointer to the new page talbe page
//
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    intptr_t *vpta = NULL;
    pte_t *pte = NULL;
    //cprintf("pgdir_walk pdx %d\n", pdx);

    //Find the page table
    if (0 == (pgdir[pdx] & PTE_P)) { // not present
        if(!create) {
            return NULL;
        }
        // alloc page table
        struct Page *pp = page_alloc(ALLOC_ZERO);
        //cprintf("\tget page 0x%x page2pa 0x%x\n",pp, page2pa(pp));
        if (pp) {
            vpta = page2kva(pp);
            pgdir[pdx] = page2pa(pp) | PTE_P;
            ++pp->pp_ref;
        }
        else { //page_alloc() failed
            return NULL;
        }
    }
    else {
    //cprintf("\told pgdir 0x%x\n",pgdir);
        pta = pgdir[pdx] & ~0xFFF;
        vpta = KADDR(pta);
        //cprintf("\tvpta 0x%x 0x%x 0x%x\n",vpta, ptx, vpta[ptx]);
    }
...
// lj FIXME: what's the elegant way?
//
// RETURNS:
//   0 on success
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
    ...
    pte_t *pte = NULL;
    uint32_t pdx = PDX(va);
    //struct Page *opp = page_lookup(pgdir, va, &pte);
